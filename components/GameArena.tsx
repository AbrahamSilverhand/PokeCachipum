    ...
    <div className="flex flex-col items-center w-full max-w-5xl mx-auto py-8 px-4">
      ...
      <div className="w-full relative flex flex-col md:flex-row items-center justify-between gap-8 mb-12 bg-white rounded-3xl p-12 shadow-2xl overflow-hidden border-8 border-pokeGold">
        ...
        <div className={`flex flex-col items-center flex-1 transition-all duration-500 ${getP1Animation()}`}>
          ...
          <h3 className="text-2xl font-black uppercase tracking-tight text-gray-800">{player1.name}</h3>
          <span className="text-pokeRed font-black flex items-center gap-2 mt-1">
            <User size={20} /> P1
          </span>
        </div>
          ...
          {(!result && !isAnimating) ? (
            <div className="bg-pokeGold text-white font-pokemon text-4xl w-24 h-24 flex items-center justify-center rounded-full border-4 border-white shadow-xl animate-pulse">
             ...
             <div className="text-center">
                <p className="font-pokemon text-pokeGold text-2xl mb-2 animate-bounce">FIN</p>
             </div>
          ) : (
      ...
      {result && (
        <div className="text-center mb-8">
          ...
          <div className="flex gap-4 justify-center">
            <button 
              onClick={resetMatch}
              className="flex items-center gap-2 bg-gray-800 text-white px-8 py-4 rounded-full font-bold hover:bg-gray-700 transition-all shadow-lg active:scale-95"
            >
              <RefreshCw size={20} /> Revancha
            </button>
            <button 
              onClick={onResetSelection}
              className="flex items-center gap-2 bg-white text-gray-800 border-4 border-gray-800 px-8 py-4 rounded-full font-bold hover:bg-gray-100 transition-all shadow-lg active:scale-95"
            >
          ...
          <div className="flex gap-6 w-full justify-center">
            {(Object.values(GameChoice) as GameChoice[]).map((choice) => (
              <button
                ...
                className={`group flex-1 flex flex-col items-center gap-3 p-6 rounded-2xl transition-all duration-300 transform hover:scale-110 active:scale-95 shadow-md hover:shadow-xl border-4 bg-white ${
              ...
              >
                ...
                <span className="font-black uppercase text-sm tracking-widest">{choice}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {isAnimating && (
        <div className="text-center py-10">
          <p className="font-pokemon text-4xl text-pokeGold animate-pulse mb-4 tracking-widest">¡ATAQUE!</p>
          <div className="flex gap-4 justify-center">
